 ...
 */
function aiosrs_check_snippet_existence(): void {
	global $post;
 ...
 * Aiosrs_enque.
 */
function aiosrs_enque(): void {
	wp_enqueue_style( 'rating_style', plugin_dir_url( __FILE__ ) . 'css/jquery.rating.css', null, '1.0' );
	wp_enqueue_script( 'jquery_rating', plugin_dir_url( __FILE__ ) . 'js/jquery.rating.min.js', [ 'jquery' ], null, false ); //phpcs:ignore:WordPress.WP.EnqueuedResourceParameters.MissingVersion
 ...
 */
function bsf_initialize_bsf_meta_boxes(): void {
...
function display_rich_snippet( $content ) {
	...
	if ( $type === '1' ) {
		...
		if ( trim( $item_review_type ) !== '' ) {
			if ( $item_review_type === 'item_event' ) {
			...
			if ( $item_review_type === 'item_software' ) {
 ...
 * Add_ajax_library.
 */
function add_ajax_library(): void {
 ...
 * Bsf_update_rating.
 */
function bsf_update_rating(): void {

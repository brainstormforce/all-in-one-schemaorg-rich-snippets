    ...
    if (!empty($video_date)) {
        ...
        if (isset($timezone)) {
            try {
                ...
                $uploadDate = $datetime->format('Y-m-d\TH:i:sP');
            } catch (Exception $e) {
...
$video .= '<script type="application/ld+json">' . json_encode($schema) . '</script>';
...
if ('' != trim($video_date)) {
    $video .= '<meta itemprop="uploadDate" content="' . esc_attr($uploadDate) . '">';
}

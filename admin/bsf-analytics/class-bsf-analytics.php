...
if ( ! class_exists( 'BSF_Analytics' ) ) {
	...
	class BSF_Analytics {
		 ...
		 * @since 1.0.0
		 */
		public function option_notice(): void {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				$time_to_display = $data['time_to_display'] ?? '+24 hours';
				$usage_doc_link  = $data['usage_doc_link'] ?? $this->usage_doc_link;

				// Don't display the notice if tracking is disabled or White Label is enabled for any of our plugins.
				if ( get_site_option( $key . '_analytics_optin', false ) !== false || $this->is_white_label_enabled( $key ) ) {
					continue;
				}
		 ...
		 * @since 1.0.0
		 */
		public function register_usage_tracking_setting(): void {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				$usage_doc_link = $data['usage_doc_link'] ?? $this->usage_doc_link;
				$author         = $data['author'] ?? 'Brainstorm Force';
		 ...
		 * @since 1.0.0
		 */
		public function render_settings_field_html( $args ): void {
			?>
			<fieldset>
		 ...
		 * @since 1.0.0
		 */
		public function update_analytics_option_callback( $old_value, $value, $option ): void {
			if ( is_multisite() ) {
		 ...
		 * @since 1.0.0
		 */
		public function add_analytics_option_callback( $option, $value ): void {
			if ( is_multisite() ) {
				$this->add_option_to_network( $option, $value );
			}
		}
		 ...
		 * @since 1.0.0
		 */
		public function maybe_track_analytics(): void {
		 ...
		 * @since 1.0.0
		 */
		public function add_option_to_network( $option, $value ): void {

			// If action coming from general settings page.
			if ( isset( $_POST['option_page'] ) && $_POST['option_page'] === 'general' ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
		 ...
		 * @since 1.0.0
		 */
		private function optin( $source ): void {
		 ...
		 * @since 1.0.0
		 */
		private function optout( $source ): void {

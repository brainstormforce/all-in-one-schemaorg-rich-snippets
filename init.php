...
class Bsf_Meta_Box {
 //phpcs:ignore:Generic.Files.OneObjectStructurePerFile.MultipleFound
	...
	public function __construct( $meta_box ) {
		...
		global $pagenow;
		if ( $upload && in_array( $pagenow, [ 'page.php', 'page-new.php', 'post.php', 'post-new.php' ], true ) ) {
			add_action( 'admin_head', [ &$this, 'add_post_enctype' ] );
		}
		add_action( 'admin_menu', [ &$this, 'add' ] );
		add_action( 'save_post', [ &$this, 'save' ] );
		add_filter( 'bsf_show_on', [ &$this, 'add_for_id' ], 10, 2 );
		add_filter( 'bsf_show_on', [ &$this, 'add_for_page_template' ], 10, 2 );
	}
	/**
	 * Add_post_enctype.
	 */
	public function add_post_enctype(): void {
		echo '
	...
	public function add(): void {
		...
		$this->_meta_box['show_on']  = empty( $this->_meta_box['show_on'] ) ? [
			'key'   => false,
			'value' => false,
	...
	public function add_for_id( $display, $meta_box ) {
		...
		$meta_box['show_on']['value'] = ! is_array( $meta_box['show_on']['value'] ) ? [ $meta_box['show_on']['value'] ] : $meta_box['show_on']['value'];
	...
	public function add_for_page_template( $display, $meta_box ) {
		...
		if ( $meta_box['show_on']['key'] !== 'page-template' ) {
			return $display;
		}
		// Get the current ID.
		if ( isset( $_GET['post'] ) ) { //phpcs:ignore:WordPress.Security.NonceVerification.Recommended
	...
	public function show(): void {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( $field['type'] === 'file' && ! isset( $field['allow'] ) ) {
				$field['allow'] = [ 'url', 'attachment' ];
			}
			if ( $field['type'] === 'file' && ! isset( $field['save_id'] ) ) {
				$field['save_id'] = false;
			}
			if ( $field['type'] === 'multicheck' ) {
				$field['multiple'] = true;
			}
			$meta = get_post_meta( $post->ID, esc_attr( $field['id'] ), $field['type'] !== 'multicheck' /* If multicheck this can be multiple values */ );
			echo '<tr class="', esc_attr( $field['class'] ),'">';
			if ( $field['type'] === 'title' || ( $field['type'] === 'select' && esc_html( $field['name'] ) === '' ) ) {
				echo '<td colspan="2">';
			} else {
			...
			switch ( $field['type'] ) {
				...
				case 'title':
					...
					break;
				case 'wysiwyg':
					wp_editor( $meta ? esc_attr( $meta ) : esc_attr( $field['std'] ), esc_attr( $field['id'] ), $field['options'] ?? [] );
					echo '<p class="bsf_metabox_description ', esc_attr( $field['class'] ),'">', esc_attr( $field['desc'] ), '</p>';
					break;
				...
				case 'file':
					...
					if ( $field['allow'] === 'url' || ( is_array( $field['allow'] ) && in_array( 'url', $field['allow'] ) ) ) {
						$input_type_url = 'text';
					}
	...
	public function save( $post_id ) {
		...
		// check permissions.
		if ( esc_attr( $_POST['post_type'] ) === 'page' ) {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! isset( $field['multiple'] ) ) {
				$field['multiple'] = $field['type'] === 'multicheck' ? true : false;
			}
			...
			if ( in_array( $field['type'], [ 'taxonomy_select', 'taxonomy_radio', 'taxonomy_multicheck' ] ) ) {
			...
			if ( isset( $field['validate_func'] ) ) {
				$ok = call_user_func( [ 'Bsf_Meta_Box_Validate', $field['validate_func'] ], $new );
 ...
 */
function bsf_scripts( $hook ): void {
	global $wp_version;
 ...
 */
function bsf_editor_footer_scripts(): void { ?>
	<?php
	if ( isset( $_GET['bsf_force_send'] ) && isset( $_GET['bsf_file_upload_nonce'] ) && wp_verify_nonce( $_GET['bsf_file_upload_nonce'], 'ajax_nonce' ) && esc_attr( $_GET['bsf_force_send'] ) === 'true' ) {
...
function bsf_force_send( $args ) {
	...
	if ( isset( $_GET['bsf_force_send'] ) && esc_attr( $_GET['bsf_force_send'] ) === 'true' ) {
		$args['send'] = true;
	}
 ...
 */
function bsf_oembed_ajax_results(): void {
	// verify our nonce.
